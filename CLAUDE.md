...
- Tests: 5,024 examples, 0 failures - measured 2026-08-06 on the lab host (Ubuntu 24.04.4 LTS x86_64, Ruby 3.2.3) against live services with TINA4_REQUIRE_SERVICES=1, at two seeds (777 and 31337) because a bare constant declared inside an `RSpec.describe` block lands on Object and can clobber another spec file at some seeds and not others. **Firebird is NOT excluded** - the previous line said it was, and that was wrong. `spec/firebird_*.rb` runs against a REAL Firebird 5.0.2; the Gemfile's `firebird` group is what makes `fb` compile, and the lab sets `BUNDLE_WITH=firebird`. A machine without `firebird-dev` is the case that skips, not the design.
- Version: 3.13.94
