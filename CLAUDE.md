...
Version 3.13.94 - TINA4: The Intelligent Native Application 4ramework. Simple. Fast. Human. Built for AI. Built for you. See https://tina4.com for full documentation.
...
- Tests: 5,046 examples, 0 failures, 0 pending - measured 2026-08-06 on the lab host (Ubuntu 24.04.4 LTS x86_64, Ruby 3.2.3) against live services with TINA4_REQUIRE_SERVICES=1, at seeds 1111 and 9999. Seeds are varied because ORDER-DEPENDENT state leaks between spec files and a single seed hides them. A bare constant declared inside an `RSpec.describe` block is the classic one (it lands on Object and clobbers another spec file), but it is NOT the only one: **`Tina4::Log` memoises its output destination and console level in class ivars**, so a spec that leaves it configured as `output = "file"` silently sends every later `Log.warning` to the log FILE instead of `$stdout` - which is exactly how a warning assertion passed at four seeds and failed at a fifth. **Two seeds is a floor, not a proof.** If a spec asserts on logger output, pin the sink for the duration and restore the exact previous values (see `with_console_logging` in `spec/database_connect_timeout_spec.rb`). **Firebird is NOT excluded** - `spec/firebird_*.rb` runs against a REAL Firebird 5.0.2; the Gemfile's `firebird` group is what makes `fb` compile, and the lab sets `BUNDLE_WITH=firebird`. A machine without `firebird-dev` is the case that skips, not the design.
- Version: 3.13.94
