...
**Before doing anything in this repo, invoke the `tina4-maintainer` skill** (Skill tool: `tina4-maintainer`, or the directory-scoped variant `tina4-ruby:tina4-maintainer`). For UI/frontend work also load `tina4-js`. Re-invoke after any context compaction.
...
Version 3.13.138 - TINA4: The Intelligent Native Application 4ramework. Simple. Fast. Human. Built for AI. Built for you. See https://tina4.com for full documentation.
...
orders.find({ "total" => { "$gt" => 5 } }).sort("total", -1).limit(10).to_a
...
**`sort` takes all three spellings, on both providers (ADR-0036).** `sort("total", -1)`, `sort({ "total" => -1 })` (a Mongo sort document) and `sort([["total", -1]])` (a list of pairs) are equivalent everywhere. Before ADR-0036 only the hash form worked on Mongo: `Mongo::Collection::View#sort` takes ONE spec, so the two-argument form the framework documents raised `ArgumentError`, and the pairs form reached the server as an array and came back `[14:TypeMismatch]`.
...
**One client per (uri, database), and a way to close it.** `get_collection` caches the `Mongo::Client` rather than building a new one per call — before 3.13.95 it constructed one on EVERY call and never closed it, so 20 calls left 60 server connections open and the count grew without bound (invisible locally, because the SQLite fallback opens no connections at all). `Tina4::DocStore.close_doc_store` closes every Mongo client and the SQLite store.
...
- Runtime gems: NONE. The gemspec declares no runtime dependency; the HTTP server
  ...
  - `sqlite3`: an APP dependency (ADR-0067), required lazily through `Tina4.require_sqlite3!`
    (`lib/tina4/sqlite3_gem.rb`), which raises an actionable LoadError when the gem is missing.
    `tina4 init ruby` and `tina4ruby init` write `gem "sqlite3"` into the app Gemfile.
    It stays a development dependency for this repo's suite
  - `net-smtp` / `net-imap`: `Tina4::Messenger::SmtpClient` and `Tina4::Messenger::ImapClient` on `socket` + `openssl`
    (`lib/tina4/mail_socket.rb` is the shared line reader with TLS). SMTP: implicit TLS on port
...
- CORS middleware
- Middleware ordering & error handling (visible-but-resilient): cross-class order = **registration order** (the order classes are attached via `Router.use` / `route.middleware`); within a class, `before_*`/`after_*` run in **source-definition order** (resolved by `source_location` line number, immune to Ruby's symbol-table reordering of `instance_methods`), inherited methods (base→derived) before a subclass's own. `before_*` run before the handler, `after_*` after. A middleware that **throws** is caught, logged via `Tina4::Log.error`, and produces a deterministic clean 500 (`{"error":"Internal Server Error","status":500}`) — never an unhandled crash. `after_*` ALWAYS run, even when a `before_*` short-circuited with status >= 400 (handler skipped) or threw, so they can still add headers/logging. **Per-route** class middleware (`route.middleware(MyMiddleware)`) runs its `before_*`/`after_*` hooks through the SAME orchestrator as global middleware — same discovery, same table; 2-arg callable ("filter") middleware and 3-arg function middleware are unaffected. **Hook return values**, at every scope: a `Tina4::Response` SHORT-CIRCUITS and IS the response at any status (the primary rule — the only one that can express a 302 redirect); `[request, response]` rebinds and continues; `false` short-circuits sending the response AS SET (403 only when it is still default/empty); `nil` continues. A status >= 400 after the hook returns also short-circuits — a retained LEGACY COMPATIBILITY path, checked unconditionally (it used to be checked only for hooks that returned the pair, so a hook that set 403 and returned nothing ran the handler anyway). Ruby's implicit return means a hook ending in a chainable response call returns that Response and therefore short-circuits — end hooks with `[request, response]` or `nil`. Block handlers (`Tina4::Middleware.before(pattern) { }`) are NOT covered by the table and keep their "`false` halts" contract. A route's middleware list also accepts **string specs** — `middleware: ["ResponseCache"]` / `["ResponseCache:300"]`, resolved by `Tina4::Router.resolve_string_middleware` (one memoised instance per spec; an unknown name raises `ArgumentError` naming the known set).
...
- WebSocket upgrade security — origin allow-list via `TINA4_WS_ALLOWED_ORIGINS` (comma-separated exact origins). Empty/unset = allow all (non-breaking, current behaviour); when set, an upgrade whose `Origin` isn't listed is refused 403. Enforced on every upgrade path (`Tina4.websocket_origin_allowed?`). Idle reaper via `TINA4_WS_IDLE_TIMEOUT` (seconds; 0/unset = disabled) — tracks last-activity per connection and closes/prunes connections idle past the timeout. Writes never block a broadcast: frames the kernel cannot take yet queue per connection and drain in the background; a client whose queue passes `TINA4_WS_MAX_BACKLOG` bytes (default 1 MiB, 0 = unbounded, same setting as tina4-nodejs) is dropped as too slow. Binary (ASCII-8BIT) payloads go out as binary frames. `Tina4::WebSocket#start(host:, port:)` is a working standalone server (`port: 0` + `#port` for an ephemeral port). WS upgrades need `rack.hijack`: the built-in server and Puma both provide it; under a Rack server without it the upgrade is rejected 426
