...
Create a server-rendered page using the Frond template engine (Twig/Jinja2 syntax).
...
Tina4::Router.get "/dashboard" do |request, response|
  response.render("pages/dashboard.twig", {
    "title" => "Dashboard",
    "stats" => [
...
- Use `{{ var | safe }}` not `{{ var | raw }}` for unescaped output
- Use `{% elif %}` not `{% elseif %}`
