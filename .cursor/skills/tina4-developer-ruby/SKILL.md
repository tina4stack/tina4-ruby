...
6. **Render a template with `response.render(name, data)`.** To render a page from a route:
   ...
   **A template global registered with `add_global(name, value)` stores your
   value as-is — pass a VALUE, not a lazy callable.** A lambda or proc is stored
   uncalled, so when you name it bare in a condition it is a truthy object:
   `{% if flag %}` is ALWAYS true when `flag` is a lambda, even `-> { false }`
   (only `{% if flag() %}` calls it). Resolve it to a real bool before you pass
   it, or call it in the template. Full note + the
