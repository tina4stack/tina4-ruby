...
module Tina4
  module ErrorOverlay
    ...
    class << self
      ...
      def render_production(status_code: 500, message: "Internal Server Error")
        <<~HTML
          <!DOCTYPE html>
          <html lang="en">
          <head>
          ...
          <style>
          *{margin:0;padding:0;box-sizing:border-box;}
          body{background:#{BG};color:#{TEXT_COLOR};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;
          display:flex;justify-content:center;align-items:center;min-height:100vh;text-align:center;}
          </style>
          </head>
          <body>
          <div>
            <h1 style="font-size:72px;color:#{RED};margin-bottom:16px;">#{status_code}</h1>
            <p style="font-size:20px;color:#{SUBTEXT};">#{esc(message)}</p>
            <p style="margin-top:24px;font-size:14px;color:#{OVERLAY_COLOR};">Tina4 Ruby</p>
          </div>
          </body>
          </html>
        HTML
