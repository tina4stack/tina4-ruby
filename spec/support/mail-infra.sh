...
# Stand up the TLS + AUTH mail servers the Messenger transport specs drive
# (spec/mail_transport_spec.rb). Idempotent: safe to re-run. The plain GreenMail
...
# DNS:localhost and IP:127.0.0.1. The specs trust that CA through SSL_CERT_FILE,
# so certificate verification stays ON; the negative examples prove a server is
# REFUSED when its CA is not trusted.
#
# There are no mocks in the mail specs: SMTP and IMAP are hand-written protocol
# clients, so they are proven on the wire against real servers, here and in CI.
#
# Usage:  bash spec/support/mail-infra.sh [dir] [name-prefix]
...
#         `bash spec/support/mail-infra.sh [dir] [name-prefix] down` removes them.
set -e
