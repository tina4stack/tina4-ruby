...
RSpec.describe "Database connected log line redaction (real PostgreSQL + ODBC)" do
  ...
  describe "PostgreSQL passwords containing ':' and '@'" do
    before do
      skip "postgres not set: TINA4_TEST_PG_URL not set" if admin_url.empty?
      begin
        require "pg"
      rescue LoadError
      ...
      begin
        ...
        skip "postgres not reachable at #{URI.parse(admin_url).host}: #{e.class}"
      end
      @roles = []
    end
  ...
  describe "ODBC PWD=" do
    before do
      skip "TINA4_TEST_ODBC_DSN not set (needs a live ODBC source)" if odbc_dsn.empty?
      skip "TINA4_TEST_ODBC_DSN carries no Pwd= - not set for this check" unless odbc_dsn.match?(/pwd=([^;]+)/i)
    end
