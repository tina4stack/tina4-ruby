...
cd my-app && bundle install && tina4ruby serve
```

SQLite is the default database. Its driver, the `sqlite3` gem, is an app dependency like `pg` or
`mysql2`: both `init` commands put `gem "sqlite3"` in the new project's Gemfile. A project without it
gets a LoadError that says to add it.
